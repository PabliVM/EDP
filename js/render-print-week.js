...
export function printWeek() {
  ...
  const html = buildFullDoc([{ teamFull, plans }], { season, microN, monday, icons, weekLabel: formatWeekRange(monday) }, false);
...
function loadTeamPlans(seasonKey, teamKey, weekId, days) {
  return new Promise((resolve, reject) => {
    ...
    // Timeout por si un equipo no tiene datos
...
function buildFullDoc(sheetsData, { season, microN, monday, icons, weekLabel }, withCover) {
  ...
  const sheetsHTML = sheetsData.map(({ teamFull, plans }) =>
    buildSheetHTML({ teamFull, plans, season, microN, monday, icons, weekLabel, logoSrc })
  ).join('');
...
<head>
  <meta charset="UTF-8" />
  <title>Planificación EDP — ${safeText(weekLabel)}</title>
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    .print-header-team  {
      ...
      color: #ffffff !important; -webkit-print-color-adjust: exact; print-color-adjust: exact;
    ...
    .print-day {
      border: 1px solid #d1d9e6; border-radius: 6px; overflow: hidden;
    ...
    .print-day-header {
      background: #f0f4fa !important; -webkit-print-color-adjust: exact; print-color-adjust: exact;
      padding: 4px 6px; border-bottom: 1px solid #d1d9e6;
    }
...
function buildCover({ weekLabel, season, logoSrc }) {
  return `
    <div class="cover page-break">
      ...
      <div class="cover-title">Coordinación EDP</div>
      <div class="cover-sub">Planificación semanal de porteros</div>
      <div class="cover-week">📅 ${safeText(weekLabel)}</div>
      <div class="cover-season">${safeText(season.name || season.seasonKey)}</div>
    </div>
  `;
}

function buildSheetHTML({ teamFull, plans, season, microN, monday, icons, weekLabel, logoSrc }) {
  const days     = getWeekDays(monday);
